(ns schoolops.governor-contract-test
...
(deftest attendance-note-logging-full-flow
  (testing "clean attendance-note proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-1" :phase 3}
          result (exec-request actor "t1"
                               {:op :log-attendance-note :student-id "student-1" :patch {:meal "lunch" :mood "good"}}
                               ctx)]
      (is (some? result))
  ...
  (testing ":flag-safety-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-safety-concern :student-id "student-1"
                                :patch {:concern "student appeared distressed" :confidence 0.99}}
                               ctx)]
      (is (some? result))
  ...
  (testing "proposal with :effect :commit (not :propose) -> hard hold"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t5"
                               {:op :log-attendance-note :student-id "student-1"
                                :patch {:meal "lunch"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into grading/pedagogical scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-attendance-note :student-id "student-1" :patch {:meal "lunch"}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-attendance-note :student-id "unknown" :patch {:meal "lunch"}}
                     ctx)
