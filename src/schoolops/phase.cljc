(ns schoolops.phase
  ...
  operations-coordination actor.
    ...
    Phase 1  assisted-logging     -- attendance-note logging allowed, every
                                     write needs human approval.
    Phase 2  assisted-coordination-- adds family visit, supply request,
                                     staff shift proposals, still approval.
    Phase 3  supervised auto      -- governor-clean, high-confidence
                                     `:log-attendance-note`/`:schedule-parent-guardian-meeting`/
                                     `:coordinate-supply-request`/
                                     `:schedule-staff-shift-proposal` may
  ...
  needs a human to actually look at it. `schoolops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently --
  two layers, not one, agree on this."
  (:require [schoolops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                                           :auto #{}}
   1 {:label "assisted-logging"       :writes #{:log-attendance-note}                                                             :auto #{}}
   2 {:label "assisted-coordination"  :writes #{:log-attendance-note :schedule-parent-guardian-meeting :coordinate-supply-request
                                               :schedule-staff-shift-proposal}                                             :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-attendance-note :schedule-parent-guardian-meeting :coordinate-supply-request :schedule-staff-shift-proposal}}})
