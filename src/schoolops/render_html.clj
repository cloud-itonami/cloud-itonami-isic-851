(ns schoolops.render-html
  ...
  (`schoolops.operation` -> `schoolops.governor` -> `schoolops.store`)
  through a scenario built from the actor's OWN seeded demo data
  (`schoolops.store/seed-db`, students student-1/student-2/student-3) and
...
(defn run-demo!
  ...
  (let [db (store/seed-db)
    ...
    (exec! actor "s1-supply" {:op :coordinate-supply-request :student-id "student-1"
                               :patch {:item "workbooks" :quantity 30}})
...
(def ^:private action-gate-rows
  ...
  ;; (README `Ops` table, `schoolops.governor`/`schoolops.phase`) --
...
(defn render
  ...
  (let [ledger (vec (store/ledger db))
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-851 &middot; primary-education operations coordination</title><style>\n"
     ...
     "</style></head><body>\n"
     "<header class=\"bar\">\n"
     "  <h1>Primary education back-office coordination (ISIC 851) — Operator Console</h1>\n"
     "  <span class=\"badge\">read-only sample · governor-gated · never touches grading/discipline/custody</span>\n"
     "</header>\n"
     "<main>\n"
     "  <section class=\"card\">\n"
     "    <h2>Students</h2>\n"
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>schoolops.store</code> via <code>schoolops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated nightly.</p>\n"
     "    <table>\n"
     ...
     "  </section>\n"
     "  <section class=\"card\">\n"
     "    <h2>Action gate (SchoolOps Governor)</h2>\n"
